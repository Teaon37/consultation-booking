...
import type { Answer, CustomerAnswers, FeedbackAnswers, StepId } from "@/types/survey";
...
export function getPreviousStep(
  ...
  if (current === "b1") return "start";
  if (current === "b2") return "b1";
  if (current === "mvp-intro") return "b9";
  if (current === "q1") return "mvp-intro";
...
export function getProgressSteps(
  ...
  const steps: StepId[] = ["start", ...CUSTOMER_QUESTIONS.map((q) => q.id), "mvp-intro"];
...
export function getProgressPercent(
  ...
  const steps = getProgressSteps(customerAnswers, feedbackAnswers);
    ...
    Object.entries(answers).map(([key, answer]) => {
      ...
      if (answer.type === "multi") {
        return [
          key,
          answer.other
            ? { values: answer.values, other: answer.other }
            : answer.values,
        ];
      }
